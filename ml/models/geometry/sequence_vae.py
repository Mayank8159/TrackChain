...
Canonical single-source-of-truth module.  Contains BOTH architectures:
  • SequenceVAE           – lightweight (96-channel fusion, 64×20 flat)
  • EnhancedSequenceVAE   – deep (192-channel fusion, 128×8 flat, BatchNorm, Huber loss)

The SequenceVAEDetector wrapper auto-detects which architecture matches the checkpoint
and raises a hard RuntimeError on any weight mismatch (NO silent random-weight fallback).
...
Features:
  ...
  4. Overlapping Window Inference for boundary defect protection
"""
...
# ---------------------------------------------------------------------------
# Architecture A: Lightweight SequenceVAE (96-channel fusion)
# ---------------------------------------------------------------------------
...
class DilatedEncoder(nn.Module):
    ...
    Lightweight variant: single conv per branch → 96-channel fusion.
    """
...
class SequenceVAE(nn.Module):
    ...
    Lightweight variant (96-channel fusion, 64×20 flat).
    """
...
# ---------------------------------------------------------------------------
# Architecture B: Enhanced SequenceVAE (192-channel fusion, BatchNorm, Huber)
# ---------------------------------------------------------------------------


class DilatedEncoder1D(nn.Module):
    """
    Multi-scale 1D CNN encoder with dilated convolutions and BatchNorm.
    Captures both short-wavelength (1-3m) and long-wavelength (10-30m) patterns.
    Enhanced variant: two conv layers per branch → 192-channel fusion.
    """
...
class EnhancedSequenceVAE(nn.Module):
    ...
    - Multi-scale dilated encoder (192-channel fusion)
...
# ---------------------------------------------------------------------------
# Production Inference Wrapper (auto-detecting architecture)
# ---------------------------------------------------------------------------
...
class SequenceVAEDetector:
    ...
    Production detector wrapper for SequenceVAE featuring:
      - Auto-detects Enhanced vs Lightweight architecture from checkpoint state dict
      - HARD mismatch guard: raises RuntimeError on weight load failure (NO silent random fallback)
        ...
        for wp in candidate_weights:
            if wp and os.path.exists(str(wp)):
                try:
                    ...
                    # Auto-detect architecture from state dict fusion layer input channels
                    ...
                    # HARD GUARD: strict=True and no fallback on failure
                    missing, unexpected = self.model.load_state_dict(state, strict=False)
                    if unexpected or any("fusion" in k for k in missing):
                        raise RuntimeError(
                            f"VAE architecture mismatch vs {wp} — "
                            f"missing keys with 'fusion': {[k for k in missing if 'fusion' in k]}, "
                            f"unexpected keys: {unexpected}. "
                            f"Do NOT fall back to random weights."
                        )
                    loaded_checkpoint = True
                    break
                except RuntimeError:
                    raise  # Re-raise our own hard guard
